...
const outUrl = new URL("./meu-treino-wireframes.excalidraw", import.meta.url);
...
function screen02(index) {
  const [x, y] = pos(index);
  const c = dark;
  add(phoneFrame(x, y, "UX-02 Inicio com treino", c), header(x, y, c));
  add(rect("plan-card", x + 32, y + 148, 326, 118, c));
  add(text("plan-title", x + 56, y + 170, "Hipertrofia 4x", c, { width: 180, size: 21 }));
  add(
  ...
  add(rect("next-card", x + 32, y + 298, 326, 246, c, {
    stroke: c.primary,
    strokeWidth: 2,
  }));
  add(pill("recommended", x + 56, y + 322, 130, "Recomendado", c));
  add(text("next-title", x + 56, y + 368, "Treino B\nSuperiores", c, {
    width: 200,
    size: 26,
    lineHeight: 1.12,
  }));
  add(
    text("next-meta", x + 56, y + 436, "42 min   6 exercicios\nDepois do Treino A", c, {
      width: 230,
      size: 15,
      color: c.muted,
      lineHeight: 1.35,
  ...
  add(button("start-workout", x + 56, y + 488, 278, "Iniciar treino", c));
  add(rect("quick-status", x + 32, y + 574, 326, 132, c));
  add(
    text(
      "quick-status-text",
      x + 56,
...
function screen03(index) {
  const [x, y] = pos(index);
  const c = dark;
  add(phoneFrame(x, y, "UX-03 Detalhe do treino", c));
  add(text("back-title", x + 24, y + 74, "< Treino B - Superiores", c, {
    width: 270,
    size: 18,
  }));
  add(rect("summary-card", x + 32, y + 124, 326, 128, c));
  add(text("summary-title", x + 56, y + 148, "Hoje: superiores", c, {
    width: 210,
    size: 22,
  }));
  add(
    text(
      ...
      x + 56,
      y + 184,
      "Aquecimento + 6 exercicios\nRIR alvo 2 | descanso 90s",
      c,
      { width: 235, size: 15, color: c.muted, lineHeight: 1.35 },
    ),
  );
  add(button("start-from-detail", x + 226, y + 182, 108, "Iniciar", c));
  add(text("section-warmup", x + 32, y + 282, "Aquecimento", c, {
    width: 180,
    size: 16,
    color: c.info,
  }));
  add(rect("warmup-row", x + 32, y + 312, 326, 56, c));
  add(text("warmup-text", x + 52, y + 329, "Mobilidade + 5 min esteira", c, {
    width: 250,
    size: 15,
    color: c.muted,
  }));
  add(text("section-exercises", x + 32, y + 396, "Exercicios", c, {
    width: 180,
    size: 16,
    color: c.info,
  }));
  add(exerciseRow("ex1", x + 32, y + 424, c, "Supino reto", "3x 8-10 | RIR alvo 2", "50 kg"));
  add(exerciseRow("ex2", x + 32, y + 510, c, "Remada baixa", "3x 10-12 | RIR alvo 2", "45 kg"));
  add(exerciseRow("ex3", x + 32, y + 596, c, "Desenvolvimento", "3x 8-10 | RIR alvo 2", "22 kg"));
  add(text("more-exercises", x + 52, y + 690, "+ 3 exercicios e cooldown", c, {
    width: 240,
    size: 15,
    color: c.weak,
  }));
  add(button("detail-primary", x + 48, y + 722, 294, "Iniciar treino", c));
...
function screen04(index) {
  const [x, y] = pos(index);
  const c = dark;
  add(phoneFrame(x, y, "UX-04 Execucao do treino", c));
  add(
    text(
      "active-head",
      x + 24,
      y + 74,
      "< Treino B - Superiores        Pausar\nExercicio 2 de 6",
      c,
      { width: 342, size: 16, lineHeight: 1.35 },
    ),
  );
  add(rect("timer-compact", x + 32, y + 140, 326, 78, c, { stroke: c.info }));
  add(text("timer-compact-copy", x + 56, y + 156, "Descanso", c, {
    width: 120,
    size: 15,
    color: c.info,
  }));
  add(text("timer-compact-time", x + 56, y + 178, "01:20", c, {
    width: 130,
    size: 31,
    color: c.info,
  }));
  add(button("skip-rest", x + 234, y + 154, 100, "Pular", c, "info"));
  add(rect("active-exercise", x + 32, y + 244, 326, 112, c));
  add(text("active-ex-title", x + 56, y + 266, "Supino reto", c, { width: 220, size: 22 }));
  add(
    text(
      "active-ex-meta",
      x + 56,
      y + 302,
      "3 series | 8-10 reps | RIR alvo 2\nUltima carga: 50 kg",
      c,
      { width: 260, size: 14, color: c.muted, lineHeight: 1.35 },
    ),
  );
  add(rect("set-card", x + 32, y + 386, 326, 254, c));
  add(text("set-title", x + 56, y + 410, "Serie 2 de 3", c, {
    width: 160,
    size: 18,
    color: c.primary,
  }));
  [
    ...
    add(text(`set-label-${idx}`, bx, y + 454, label, c, {
      width: 76,
      size: 13,
      color: c.muted,
      align: "center",
    }));
    add(rect(`set-value-${idx}`, bx, y + 480, 76, 54, c, {
      fill: c.elevated,
      stroke: c.border,
    }));
    add(text(`set-value-text-${idx}`, bx + 4, y + 494, value, c, {
      width: 68,
      size: 22,
      align: "center",
    }));
    add(rect(`minus-${idx}`, bx, y + 548, 34, 34, c, { fill: "transparent" }));
    add(text(`minus-text-${idx}`, bx + 9, y + 553, "-", c, {
      width: 16,
      size: 18,
      align: "center",
      color: c.muted,
    }));
    add(rect(`plus-${idx}`, bx + 42, y + 548, 34, 34, c, { fill: "transparent" }));
    add(text(`plus-text-${idx}`, bx + 50, y + 553, "+", c, {
      width: 18,
      size: 18,
      align: "center",
      color: c.primary,
  ...
  add(text("set-row-1", x + 56, y + 602, "Serie 1 concluida   50 kg   10   RIR 2", c, {
    width: 270,
    size: 13,
    color: c.weak,
  }));
  add(button("save-set", x + 32, y + 674, 326, "Salvar serie", c));
  add(button("next-exercise", x + 32, y + 742, 326, "Proximo exercicio", c, "secondary"));
}
...
function screen09(index) {
  const [x, y] = pos(index);
  const c = dark;
  add(phoneFrame(x, y, "UX-09 Importar JSON", c), header(x, y, c, "Importar", "Offline pronto"));
  ...
  add(
    text(
      ...
      x + 70,
      y + 330,
      "O arquivo sera validado antes de substituir o treino atual.",
      c,
  ...
  add(button("model-shortcut", x + 48, y + 706, 294, "Baixar modelo JSON", c, "secondary"));
  add(nav(x, y, c, "Treino"));
}
...
function screen12(index) {
  const [x, y] = pos(index);
  const c = dark;
  add(phoneFrame(x, y, "UX-12 Baixar modelo JSON", c), header(x, y, c, "Modelo JSON", "Dados locais"));
  ...
  add(
    text(
      ...
      x + 62,
      y + 330,
      "Baixe o arquivo e envie para a IA ou professor preencher o novo plano.",
      c,
      { width: 266, size: 14, align: "center", color: c.muted, lineHeight: 1.35 },
    ),
  );
...
function screen13(index) {
  const [x, y] = pos(index);
  const c = dark;
  add(phoneFrame(x, y, "UX-13 Configuracoes", c), header(x, y, c, "Ajustes", "v0.1.0"));
  add(rect("theme-settings", x + 32, y + 154, 326, 138, c));
  add(text("theme-title", x + 56, y + 178, "Tema", c, { width: 160, size: 20 }));
  add(text("theme-copy", x + 56, y + 210, "Preferencia salva no aparelho", c, {
    width: 220,
    size: 14,
    color: c.muted,
  }));
  add(rect("theme-dark", x + 56, y + 244, 118, 34, c, {
    fill: c.primary,
    stroke: c.primary,
  }));
  add(text("theme-dark-label", x + 78, y + 253, "Escuro", c, {
    width: 74,
    size: 13,
    align: "center",
    color: c.inkOnPrimary,
  }));
  add(rect("theme-light", x + 184, y + 244, 118, 34, c, { fill: "transparent" }));
  add(text("theme-light-label", x + 206, y + 253, "Claro", c, {
    width: 74,
    size: 13,
    align: "center",
    color: c.text,
  }));
  add(rect("data-settings", x + 32, y + 326, 326, 196, c));
  add(text("data-title", x + 56, y + 350, "Dados locais", c, { width: 180, size: 20 }));
  add(
    text(
      "data-copy",
      x + 56,
      ...
      c,
      { width: 260, size: 16, color: c.muted, lineHeight: 1.7 },
    ),
  );
  add(rect("about-settings", x + 32, y + 556, 326, 116, c));
  add(
    text(
      "about-copy",
      x + 56,
      y + 580,
      "App offline-first\nPWA pronto para Android\nSem conta ou servidor",
      c,
      { width: 260, size: 15, color: c.muted, lineHeight: 1.45 },
    ),
  );
  add(button("danger-clear", x + 48, y + 704, 294, "Apagar dados locais", c, "danger"));
  add(nav(x, y, c, "Ajustes"));
}
